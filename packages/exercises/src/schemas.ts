 ...
 *
 * Moved out of apps/web/src/lib/exercises/schemas.ts in the API-boundary
 * sprint so that BOTH apps/web (renderers, CLI generators) and apps/api
 * (the /ai/generate-exercise endpoint) can validate generated exercises
 * against one canonical source. The module is pure: it depends only on
 * `zod` and the `ExerciseType` enum type from @wortschatz/database.
 *
