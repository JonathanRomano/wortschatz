/**
 * Validation gate shared by both providers. Reuses the canonical Zod
 * schemas from src/lib/exercises/schemas.ts (never duplicate them), then
 * layers type-specific structural checks the schema can't express, and
 * finally requires an English explanation (other locales are optional —
 * the renderer falls back to `en` via pickLocalized()).
 */
