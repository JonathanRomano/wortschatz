...
describe("ai wrapper — stub path (ANTHROPIC_API_KEY missing)", () => {
  ...
  // Stub-path tests for evaluateAnswer + reviewText moved to apps/api
  // alongside their implementations during Sprint 03 Task 7.
});
...
describe("ai wrapper — real path (ANTHROPIC_API_KEY set, SDK mocked)", () => {
  ...
  // evaluateAnswer + reviewText describes moved to apps/api alongside
  // their implementations during Sprint 03 Task 7. The web-side surface
  // is now a thin api-client fetch (see src/lib/__tests__/api-client.test.ts).
});
